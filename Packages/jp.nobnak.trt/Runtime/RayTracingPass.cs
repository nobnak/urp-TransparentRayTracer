...
public class RayTracingPass : ScriptableRenderPass {
    ...
    const string AlphaCompositeBlitShaderName = "Hidden/URP-RayTracer/BlitAlphaComposite";

    static Material s_alphaCompositeBlitMaterial;
    ...
    public override void RecordRenderGraph(RenderGraph renderGraph, ContextContainer frameContext) {
        ...
        using (var builder = renderGraph.AddUnsafePass<PassData>(PassName, out var passData)) {
            ...
            builder.UseTexture(passData.rtOutputTexture, AccessFlags.Read);
    ...
    static void ExecutePass(PassData data, UnsafeGraphContext context) {
        ...
        Material blitMat = GetAlphaCompositeBlitMaterial();
        if (blitMat != null)
            Blitter.BlitTexture(nativeCmd, data.rtOutputTexture, new Vector4(1, 1, 0, 0), blitMat, 0);
        else
            Blitter.BlitTexture(nativeCmd, data.rtOutputTexture, new Vector4(1, 1, 0, 0), 0, false);
    }

    static Material GetAlphaCompositeBlitMaterial() {
        if (s_alphaCompositeBlitMaterial != null) return s_alphaCompositeBlitMaterial;
        var shader = Shader.Find(AlphaCompositeBlitShaderName);
        if (shader == null) return null;
        s_alphaCompositeBlitMaterial = new Material(shader);
        s_alphaCompositeBlitMaterial.hideFlags = HideFlags.HideAndDontSave;
        return s_alphaCompositeBlitMaterial;
    }
